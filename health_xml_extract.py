...
data = HealthDataExtractor(path="export.xml", dest_path="data")
data.report_stats()
data.extract()
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: %s <path to export.xml> [verbose]" % sys.argv[0])
        sys.exit(1)
    verbose = len(sys.argv) > 2 and sys.argv[2] == "verbose"
    data = HealthDataExtractor(sys.argv[1], verbose=verbose)
    data.report_stats()
    data.extract()
